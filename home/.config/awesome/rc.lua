...
package.loaded["naughty.dbus"] = {} -- hack to disable them
...
local theme_path = gears.filesystem.get_configuration_dir() .. "themes/xresources/theme.lua"
...
local function makebatwidget()
    ...
    local files = {
        io.open("/sys/class/power_supply/BAT0/capacity"),
        io.open("/sys/class/power_supply/BAT1/capacity")
    }
    ...
    local t = gears.timer {
        ...
        callback  = function ()
            ...
            for i, file in ipairs(files) do
                file:seek("set", 0)
                table.insert(parts, file:read())
            end
            widget.text = table.concat(parts, ",")
        end
    }
    widget:connect_signal("button::press", function()
        local f = io.popen("acpi")
        naughty.notify({
            text = f:read("*a"):sub(1, -2)
        })
        f:close()
...
clientkeys = gears.table.join(
    ...
    awful.key({ modkey,           }, "Return",
              function (c)
                  local c2 = awful.client.getmaster()
                  if c == c2 then
                    c2 = awful.client.next(1, c2)
                  end
                  c:swap(c2)
              end,
